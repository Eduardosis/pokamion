...
class PokemonSummary_Scene
  ...
  def drawEnhancedToggleHint
    return if !$game_switches[Settings::ENHANCED_STATS_SWITCH]
    overlay = @sprites["overlay"].bitmap
    font    = overlay.font
    original_size = font.size
    font.size = [(original_size * 6) / 8, 1].max
    base    = Color.new(255, 18, 18)
    # Make the shadow extremely subtle
    shadow  = Color.new(0, 0, 0, 55)
    textpos = [
      [_INTL("Press USE to see IV/EV"), 102, 26, :left, base, shadow]
    ]
